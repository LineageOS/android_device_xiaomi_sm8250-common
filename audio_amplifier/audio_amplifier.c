...
#define MAX_AMPLIFIER_DEVICES 0
...
static void amplifier_open_devices() {
    ...
    for (i = 0; i < MAX_AMPLIFIER_DEVICES; i++) {
        ...
        if (rc) {
            ALOGV("%s: Failed to obtain reference to amplifier module, class %s: %s\n",
        ...
        if (rc) {
            ALOGV("%s: Failed to open amplifier hardware device, class %s: %s\n",
                    __func__, amp_devices_list[i], strerror(-rc));
            amp_devices[i] = NULL;
            continue;
        }
